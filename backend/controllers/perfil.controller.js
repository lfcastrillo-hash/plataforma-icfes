  ...
  try {
    ...
    const resultado = await pool.query(`
      SELECT 
        ...
        u.biografia,
        COALESCE(
          (SELECT json_agg(c.nombre_clase) FROM clases c WHERE c.id_profesor = u.id_usuario), 
          '[]'::json
        ) AS lista_clases
      FROM usuarios u 
...
const actualizarPerfil = async (req, res) => {
  try {
    ...
    if (nuevo_correo) {
      const existe = await pool.query("SELECT id_usuario FROM usuarios WHERE email = $1 AND id_usuario != $2", [nuevo_correo, id_usuario]);
      if (existe.rows.length > 0) {
        return res.status(400).json({ success: false, mensaje: "Ese correo ya está registrado por otro usuario." });
      }
    }

    const usuarioActual = await pool.query("SELECT foto_perfil, email, biografia FROM usuarios WHERE id_usuario = $1", [id_usuario]);
    if (usuarioActual.rows.length === 0) return res.status(404).json({ success: false, mensaje: "Usuario no encontrado" });

    let fotoUrl = usuarioActual.rows[0].foto_perfil;
    let correoFinal = nuevo_correo || usuarioActual.rows[0].email; 
    let biografiaFinal = biografia !== undefined ? biografia : usuarioActual.rows[0].biografia;
    ...
    // =========================================================
    let sqlQuery = "UPDATE usuarios SET nombre = $1, email = $2, foto_perfil = $3, biografia = $4";
